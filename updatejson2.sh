...
for _filename in $(ls *.json); do
    ...
    if [ $isTreeInYaml -gt 0 ]; then
       ...
       #Retrieve the information from the meta section, we want to remove some sensitive informations there
       currentTreeVars=$(compgen -A variable | grep "^CONF2_${currentTreeName}_meta_")

       for key in $currentTreeVars; do
                  ...
                  keyName=$(echo $key | sed -e "s/^CONF2_${currentTreeName}_meta_//")

                  if [ "$MODE" == "delete" ]; then

                       jq ".\"meta\".\"$keyName\"=\"null\"" $_filename > ${_filename}.tmp && mv ${_filename}.tmp ${_filename}

                  elif [ "$MODE" == "add" ]; then
                       
                       jq ".\"meta\".\"$keyName\"=\"$currentkey\"" $_filename > ${_filename}.tmp && mv ${_filename}.tmp ${_filename}
                       
                       currentValue=$($JSON/getSecretValue.sh "$YAML1" "$ENV" "$currentkey")
                       
                       if [ "$keyName" == "minimumPasswordLength" ]; then
                          jq ".\"meta\".\"$keyName\"=$currentValue" $_filename > ${_filename}.tmp && mv ${_filename}.tmp ${_filename}
                       else
                          jq ".\"meta\".\"$keyName\"=\"$currentValue\"" $_filename > ${_filename}.tmp && mv ${_filename}.tmp ${_filename}
                       fi
                  fi
